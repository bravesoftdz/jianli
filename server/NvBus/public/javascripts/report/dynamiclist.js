...
//	var p = JSON.stringify(params);
	window.location="/excel/report/list?"+"&formkey="+form.formkey+"&url="+form.listtitle;
...
function getPage(page, rows, isPagination){
	...
	var params = getSearchParams();
	var orderCondition = getOrderParam();
	var d = null
    $.ajax({
        url: "/data/report/page",
        type: "POST",
        async: false,
		dataType: 'JSON',
        data: {
...
function getOrderParam(){
	var $elements = $('#orderform :input[name]:enabled');
	var params = new Object();
	$.each($elements,function(){
		var name = $(this).attr('name');
		var value;
...
function getColumns(){
	...
	var columns1 =[];
	$.ajax({
		url: "/ui/report/refurbishDynamicList",
		type: "POST",
	    async: false,
		dataType: 'text',
	    data: {
...
//	var d = null;
	$.cookie("params",params,{path:"/"});
//	var p = JSON.stringify(params);
	window.location="/excel/report/print?"+"&formkey="+form.formkey+"&url="+form.listtitle;
