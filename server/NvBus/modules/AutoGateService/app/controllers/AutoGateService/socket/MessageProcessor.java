...
import controllers.AutoGateService.service.impl.CheckTicketAutoGateServiceImpl;
import util.TransactionUtil;
import util.AutoGateService.AutoGateDESUtil;
import util.AutoGateService.AutoGateNewDESUtil;
...
//	private static final String encrykey = "bus36501";
	...
	private CheckTicketAutoGateServiceImpl checkTicketAutoGateService = new CheckTicketAutoGateServiceImpl();
	...
	public Message analyzeMsg(String msgstr) {
		...
		Organization org = checkTicketAutoGateService.getOrganization(MsgHelper.CheckStationid);//供查参数等使用
		//闸机报文格式 ,0新老格式都支持,1只支持新格式,2只支持老格式
		String flag = parameterService.findParamValue("3070", org.getId());
		...
		if("END".equals(ss[5])){
			...
			msg.setGateplacetype(ss[6]);//检票条件3  闸机使用场景类型    未设置/0：检票口上车;   1：侯车厅进站  
			...
			if((!"bus365".equals(ss[7]))){
				...
				try {
					transencrypstr = AutoGateDESUtil.generateEncrypt(datasourcestr, MsgHelper.Key);
				} catch (Exception e) {
					e.printStackTrace();
				...
				if(!transencrypstr.equals(ss[7])){
					...
					try {
						new_transencrypstr = AutoGateNewDESUtil.generateEncrypt(datasourcestr, MsgHelper.Key);
					}catch(Exception e){
						e.printStackTrace();
						play.Logger.error("加密错误"+e.getMessage());
					}
