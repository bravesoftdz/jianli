...
public class MultiUnitBalancePlugin implements DSPluginInterface{
	...
	public void prepareTempTable(String params) {
        ...
        String str = "{call reportchainbalance('"+departdatebegin+"','"+departdateend+"','"+orgids+"','"+workovertimes+"','"+vehicleworktypes+"',"+unitid+","+balanceunitid+","+vehicleid+",'"+tempTable_name+"')}";
	...
	public String getSql(String params, boolean isPagination) {
		...
		tempTable_name="mubp_" + sessionid.hashCode();
	...
	public String getSummarysql(String params) {
		String str = "select cast('合计' as varchar(10)),sum(departnum),sum(departamount),sum(stationfee),sum(agentfee),sum(upfloatamount),sum(upfloatamount3),sum(balancemoney) from "+ tempTable_name;
		return str;
	}
