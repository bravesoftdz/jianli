...
public class HolidayBalanceSummaryPlugin implements DSPluginInterface{
String tempTable_name ="";
	...
	public void prepareTempTable(String params) {
        ...
        String str = "{call reportbalanceupfloat('"+departdatebegin+"','"+departdateend+"','"+orgids+"','"+workovertimes+"','"+vehicleworktypes+"',"+unitid+","+balanceunitid+","+vehicleid+","+routeid+",'"+tempTable_name+"')}";
	...
	public String getSql(String params, boolean isPagination) {
		...
		tempTable_name="hobs_" + sessionid.hashCode();
	...
	public String getSummarysql(String params) {
		String str = "select cast('合计' as varchar(10)),sum(departinvoicescount),sum(departnum),sum(departamount),sum(stationfee),sum(agentfee),sum(upfloatamount),sum(upfloatamount6),sum(tingtafee),sum(anjianfee),sum(qianzhenfee),sum(fakuanfee),sum(otherfee),sum(totalfee),sum(needbalancemoney),sum(balancemoney) from "+ tempTable_name;
		return str;
	}
