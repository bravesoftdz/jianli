...
public class UIBP implements UIInterface{
	...
	private String getHeaderWithin2(MetaColumn column) throws Exception {
		...
		for(MetaSubcolumn obj : columnlist){
			if(CommonUtil.isNotEmptyString(obj.ptitle)){
				if (map.containsKey(obj.ptitle)) {
					if(null == mapb.get(obj.ptitle)){
						mapb.put(obj.ptitle, Boolean.FALSE);
					}
					if(!mapb.get(obj.ptitle)){
						mapb.put(obj.ptitle, Boolean.TRUE);
						sb.append("{title:\"").append(obj.ptitle)
						.append("\",colspan:")
						.append(map.get(obj.ptitle).size()).append("},");
					}
				}
			}else{
	...
	private String selectFormatterFunction(MetaSubcolumn col) throws Exception{
		...
		if(type == AppContrast.INPUT_SELECT){
			...
			if(null != global){
				orgids = global.getOrgIDs();
			}
			String sql = col.digitsql;

			if (col.iscontrol && CommonUtil.isNotEmptyString(orgids)) {//权限控制字段机构过滤
				sql += " and org.id in (" + orgids + ")";
			}
...
//				sb.append("<option value=\"\">请选择</option>");
			...
			try {
				...
				//注释掉的权限
				if (obj.iscontrol && CommonUtil.isNotEmptyString(orgids)) {//权限控制字段机构过滤
					sql += " and org.id in (" + orgids + ")";
				}
				Query query = JPA.em().createNativeQuery(sql);
				List<?> list = query.getResultList();
				...
				for(Object item : list){
					arr = (Object[]) item;
					if("org!value".equals(str)){
						sb.append("<option value=\"").append(arr[2]).append("\"");//机构查询传value字段
					}else{
	...
	private Map<String,Object> setOptions(String prefix,MetaSubcolumn obj,String suffix){
		...
		if(obj.edittype.intValue() == AppContrast.INPUT_DATE){
			...
			MetaReference refer = MetaUtils.findReferBySubid(obj.id);
			map.put("id", idstr);
	...
	public String orderHtml(String formkey) {
		MetaForm form = MetaUtils.findFormByKey(formkey);
		List<MetaSubcolumn> sublist = MetaSubcolumn.find("from MetaSubcolumn where columnid = ? and listisdisplay=1 order by listlocation", form.columnid).fetch();
		StringBuilder sb = new StringBuilder();
		String fieldname = null;
		...
		for(MetaSubcolumn obj : sublist) {
			...
			String inputid = "e_" + fieldname;
				...
				.append("</label>")
				.append("<div class=\"col-xs-").append(inputwidth).append("\">")
				.append(orderfieldHtml(inputid, obj))
				.append("</div>")
				.append("</div>")
				.append("</div>");
	...
	private String orderfieldHtml(String inputid,MetaSubcolumn obj) {
		StringBuilder sb = null;
		String str = "";
		if(obj.field.contains("__")){
			str = obj.field.split("__")[0];
	...
	public String defrowdata(String formkey) {
		MetaColumn column = MetaUtils.findSecColumnByFormkey(formkey);
