...
var
  nDelphi: integer;
begin
  nDelphi := FCommDataset.CompareBookmarks(Pointer(Integer(Bookmark1)), Pointer(Integer(Bookmark2)));
  if nDelphi = -1 then
...
begin
  Result := OleVariant(LongInt(Pointer(FCommDataset.GetBookmark)));
  // Result := FCommDataset.GetBookmark; //Delphi XE2
end;
...
begin
 {  try
  ...
  FCommDataset.GotoBookmark(TBookMark(Pointer(LongInt(pvBookmark))));
end;
...
begin
  if FInnerField.IsNull then
    Result := Null
  else
    Result := OleVariant(FInnerField.Value);
end;
