...
unit NvPrnDbgeh {$IFDEF CIL} platform{$ENDIF};
...
//    property ColumnSpace:Currency read FColumnSpace write SetColumnSpace;
  ...
  private
    ...
    procedure SetCenterFont(const Value: TFont);
  public
    procedure Assign(Source: TPersistent); override;
    constructor Create;
  ...
  published
    ...
    property CenterFont: TFont read FCenterFont write SetCenterFont;//ÐÂÔö
  end;

  TNvPrintDBGridEh = class(TComponent)
  private
    ...
    procedure SetPrintGridHeadFont(const Value: TFont);
  protected
  ...
  published
    ...
    property PrintGridHeadFont:TFont read FPrintGridHeadFont write SetPrintGridHeadFont;   // Ìí¼Ó±êÌâ×ÖÌå
  end;
...
procedure Register;
begin
  RegisterComponents('Nova', [TNvPrintDBGridEh]);
end;
  ...
  if CalcRect = False then begin
    ...
    if (Layout <> tlTop) {and (MultyL = True)} then
      txth := WindowsDrawTextEx(ACanvas.Handle, Text, Length(Text), {}
        rect1, DrawFlag or DT_CALCRECT, lpDTP) // Ïîëó÷èòü êâàäðà?
    else txth := 0;
    rect1 := ARect; {}
    InflateRect(rect1, -DX, -DY);
...
constructor TNvPrintDBGridEh.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FPage := TPageParams.Create;
  FPrintGridHeadFont := TFont.Create;
  if AOwner is TDBGridEh then
  begin
    DBGridEh:=(AOwner as TDBGridEh)
  end;
  if Assigned(DBGridEh) then
    FPrintGridHeadFont.Assign(DBGridEh.TitleFont);
...
destructor TNvPrintDBGridEh.Destroy;
begin
...
procedure TNvPrintDBGridEh.Preview;
begin
...
procedure TNvPrintDBGridEh.PrintTo(VPrinter: TVirtualPrinter);
type
...
const Macros: array[0..5] of String = ('&[Page]', '&[ShortDate]', '&[Date]',
  ...
  var idx: Integer;
      old_afterscorll:TDataSetNotifyEvent;
  begin
    ...
    begin
      ...
      begin
        old_afterscorll:=DBGridEh.DataSource.DataSet.AfterScroll;
        DBGridEh.DataSource.DataSet.AfterScroll:=nil;
        try
          DBGridEh.DataSource.DataSet.Next;
        finally
          DBGridEh.DataSource.DataSet.AfterScroll:=old_afterscorll;
        end;
...
//      PrnColumnWidths.Add(Pointer(Round((PrnColumns[i].Width + 1) * fScaleX)));
    ...
    begin
      ...
      begin
        ...
        begin
          VPrinter_Canvas.Font :=FPrintGridHeadFont;//Ê¹ÓÃ×Ô¶¨ÒåµÄFont  // PrnColumns[i].Title.Font;
...
//        SelectClipRgn(Handle, 0);
  ...
  procedure DrawTreeArea(Canvas: TCanvas; ARect: TRect);
    ...
    begin
      ...
      DrawTreeElement(Canvas, ARect, TreeElement, False, fScaleX, fScaleY, DBGridEh.UseRightToLeftAlignment);
//        (DefaultRowHeight mod 2 = 1) and (TopRow mod 2 = 1));
  ...
  begin
    ...
    begin // On width of page
      ...
      if GridWidth > (RealRight - RealLeft) then
      begin
        DataPrintRec.Right := MulDiv(DataPrintRec.Right, GridWidth, RealRight - RealLeft); // Îñòàâèòü ïîëå ñòàðîã?ðàçìåð?
  ...
  begin
    ...
    if IsHeader
      ...
      begin
        pc := PageFooter;
        //pc.FFont.Size:=16;
        pc.CenterFont.Size:=12;
        Lay := tlBottom;
      end;

    VPrinter_Canvas.Font := pc.CenterFont;//pc.Font;
    if (PrintFontName <> '') then VPrinter_Canvas.Font.Name := PrintFontName;
...
//     fPrnPhysOffSetY := GetDeviceCaps(VPrinter_Canvas.Handle,PHYSICALOFFSETY);
    ...
    PenW := Trunc((fLogPelsX2 + fLogPelsY2) / 200); // PenWidth = 0.01 Inche
     ...
     {TopFooterPos := DataPrintRec.Bottom - Round(fLogPelsX2 / 4);  // 1/4 äþéì?
...
{$ENDIF}
    ...
    if (Title.Text <> '') then begin
      VPrinter_Canvas.Font := FPrintGridHeadFont;// Ê¹ÓÃ×Ô¶¨ÒåµÄFont//DBGridEh.TitleFont;
      if (PrintFontName <> '') then VPrinter_Canvas.Font.Name := PrintFontName;
  ...
  var i, RightSide: Integer;
    ...
    begin
      ...
      VPrinter_Canvas.Font := FPrintGridHeadFont;//mod  //DBGridEh.TitleFont;
      if (PrintFontName <> '')
        then VPrinter_Canvas.Font.Name := PrintFontName;
      if pghColored in Options
        then VPrinter_Canvas.Brush.Color := DBGridEh.FixedColor;
  ...
  begin
    ...
    begin
      ...
      VPrinter_Canvas.Font := FPrintGridHeadFont;//DBGridEh.Columns[i].Title.Font;
      if (PrintFontName <> '') then VPrinter_Canvas.Font.Name := PrintFontName;
...
{$ENDIF}
  ...
  begin
    if (Title.Text <> '') then
    begin
      VPrinter_Canvas.Font := FPrintGridHeadFont;//DBGridEh.TitleFont;
      if (PrintFontName <> '')
        then VPrinter_Canvas.Font.Name := PrintFontName;
  ...
  begin
    ...
    if htLast.Column <> nil
      then VPrinter_Canvas.Font := FPrintGridHeadFont//htLast.Column.Title.Font
      else VPrinter_Canvas.Font := FPrintGridHeadFont;//DBGridEh.TitleFont;
    if (PrintFontName <> '')
      then VPrinter_Canvas.Font.Name := PrintFontName;
    maxh := 0;
    ...
    begin
      ...
      if htLast.Column <> nil then
        VPrinter_Canvas.Font := FPrintGridHeadFont;//htLast.Column.Title.Font;
      if (htLast.Column <> nil) and (htLast.Column.Title.Orientation = tohVertical) then
    ...
    ANode.HeightPrn := maxh; //ñîõðàíÿåì âûñîòó ChildTree ?Õîñò?
    Result := maxh;
  end;
...
{$ENDIF}
    ...
    for i := 0 to PrnColumns.Count - 1 do
    begin
      VPrinter_Canvas.Font := FPrintGridHeadFont;// PrnColumns[i].Title.Font;
      if (PrintFontName <> '') then VPrinter_Canvas.Font.Name := PrintFontName;
...
procedure TNvPrintDBGridEh.Print;
begin
  PrintTo(VirtualPrinter);
end;

procedure TNvPrintDBGridEh.SetDBGridEh(const Value: TDBGridEh);
begin
...
procedure TNvPrintDBGridEh.SetOptions(const Value: TPrintDBGridEhOptions);
begin
  FOptions := Value;
end;

procedure TNvPrintDBGridEh.SetPage(const Value: TPageParams);
begin
  FPage := Value;
end;

procedure TNvPrintDBGridEh.SetPageFooter(const Value: TPageColontitle);
begin
  FPageFooter := Value;
end;

procedure TNvPrintDBGridEh.SetPageHeader(const Value: TPageColontitle);
begin
  FPageHeader := Value;
end;

procedure TNvPrintDBGridEh.SetPrintFontName(const Value: String);
begin
  FPrintFontName := Value;
end;

procedure TNvPrintDBGridEh.SetPrintGridHeadFont(const Value: TFont);
begin
  FPrintGridHeadFont := Value;
end;

procedure TNvPrintDBGridEh.SetTitle(const Value: TStrings);
begin
  FTitle.Assign(Value);
end;

procedure TNvPrintDBGridEh.SetUnits(const Value: TMeasureUnits);
var AMul: Currency;
begin
...
function TNvPrintDBGridEh.PrinterSetupDialog: Boolean;
begin
  Result := False;
...
procedure TNvPrintDBGridEh.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
  if (Operation = opRemove) and (AComponent = DBGridEh) then DBGridEh := nil;
end;

procedure TNvPrintDBGridEh.PrinterSetupDialogPreview(Sender: TObject);
begin
  if PrinterSetupDialog then
    Preview;
end;

procedure TNvPrintDBGridEh.SetBeforeGridText(const Value: TStrings);
var
...
procedure TNvPrintDBGridEh.DefineProperties(Filer: TFiler);
begin
...
procedure TNvPrintDBGridEh.ReadBeforeGridText(Stream: TStream);
begin
  BeforeGridText.LoadFromStream(Stream);
end;

procedure TNvPrintDBGridEh.WriteBeforeGridText(Stream: TStream);
begin
  BeforeGridText.SaveToStream(Stream);
end;

procedure TNvPrintDBGridEh.SetAfterGridText(const Value: TStrings);
var
  Stream: TStringStream;
begin
  Stream := TStringStream.Create('');
...
procedure TNvPrintDBGridEh.ReadAfterGridText(Stream: TStream);
begin
  AfterGridText.LoadFromStream(Stream);
end;

procedure TNvPrintDBGridEh.WriteAfterGridText(Stream: TStream);
begin
  AfterGridText.SaveToStream(Stream);
end;

function TNvPrintDBGridEh.GetAfterGridText: TStrings;
begin
...
function TNvPrintDBGridEh.GetBeforeGridText: TStrings;
begin
...
function TNvPrintDBGridEh.GridTextReplace(RichStrings: TStrings;
...
procedure TNvPrintDBGridEh.SetSubstitutes(ASubstitutes: array of const);
var i: Integer;
begin
...
procedure TPageColontitle.SetCenterFont(const Value: TFont);
begin
  FCenterFont := Value;
end;
