...
type
  ...
  private
    ...
    FSaveCurY: Extended;         { global for columns }
        ...
        begin
          ...
          if b.FGroup <> nil then
            if FirstTime then
              ...
              CheckGroups(b, TfrxGroupHeader(b.FGroup), ColumnKeepPos, FSaveCurY);
          ...
          Inc(b.FLineN);
          Inc(b.FLineThrough);
          CheckBandColumns(b, ColumnKeepPos, FSaveCurY);
          AddBandOutline(b);
          ShowBandTree(b);
...
begin
  ...
  begin
    if Band.KeepChild then
      StartKeep(Band);
    DoShow(Band);
    if (Band is TfrxDataBand) and (TfrxDataBand(Band).CurColumn > 1) then { dont shof childs for columns}
      chBand := nil
    else
      chBand := Band.Child;
