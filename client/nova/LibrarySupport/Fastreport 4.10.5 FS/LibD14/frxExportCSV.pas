...
var
  ...
  function PrepareString(const Str: AnsiString): AnsiString;
    ...
    { Returns count of a character encounters within a byte string }

    function CharCount(s: PAnsiChar; c: AnsiChar): LongWord; cdecl;
    begin
      Result := 0;
      if s^ = #0 then
        Exit;
    ...
    procedure DoubleCharacter(dest, src: PAnsiChar; c: AnsiChar); cdecl;
    asm
      push  esi
      push  edi
    ...
    function FindSpecChar(s: PAnsiChar; c: AnsiChar): Boolean; cdecl;
    asm
      mov   Result, 0
      push  esi
      mov   esi, s
    ...
    function CleanSysSYmbols(s: PAnsiChar): LongWord; cdecl;
    asm
      push  esi
      push  edi
  ...
  begin
    ...
    if not FindSpecChar(@s[1], sep) then
    begin
    ...
    { If the text contains special characters,
      escape all double quotes with auxiliary double quotes and
