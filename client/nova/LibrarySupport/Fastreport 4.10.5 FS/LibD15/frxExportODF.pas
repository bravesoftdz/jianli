...
uses frxUtils, frxFileUtils, frxUnicodeUtils, frxRes, frxrcExports, frxGraphicUtils;
...
var
  XML: TfrxXMLDocument;
  f: TFileStream;
  s, s1, s2, s3: WideString;
  FList: TStringList;
  i, j, k, x, y, Page, PicCount: Integer;
  dx, dy, fx, fy: Integer;
  Style: TfrxIEMStyle;
  d: Extended;
  Obj: TfrxIEMObject;
  l : integer;
  picNode, node: TfrxXMLItem;
  sl: TStringList;

  styleNode:  TfrxXMLItem;    // this node contains ODF styles
  styles:     TList;          // List of TSpanStyle
  ...
  begin
    ...
    s.Color := Color;
    s.Style := Style;
    s.Name  := 'ss-' + IntToStr( styles.Count );
    ...
    begin
      ...
      // todo: struck-out text and colored text
    end;

    Result := s.Name;
  end;

  procedure AddPic(Node: TfrxXMLItem);
  begin
    Inc(PicCount);
    with Node do
    begin
                        ...
                        with Obj.Style.DisplayFormat do
                          ...
                          begin
                            ...
                            begin
                              ...
                              node.Name := 'text:p';
                              if FExportType = 'text' then
                                node.Prop['text:style-name'] := 'p' + IntToStr(Obj.StyleIndex);

                              if Obj.HTMLTags then
                                SplitOnTags(node, Obj, sl[k])
                              else
                                node.Value := sl[k];
                            end;
                          ...
                          begin
                            ...
                            for l := 1 to Length(s) do
                              if (s[l] >= '0') and (s[l] <= '9') or (s[l] = '-') then
                                ...
                                if (Char(s[l]) = SysUtils.DecimalSeparator) or (s[l] = DecimalSeparator) then
                                  s2 := s2 + SysUtils.DecimalSeparator;

                            Prop['office:value-type'] := 'float';
                            Prop['office:value'] := StringReplace(s2,
                              SysUtils.DecimalSeparator, '.', [rfReplaceAll]);
