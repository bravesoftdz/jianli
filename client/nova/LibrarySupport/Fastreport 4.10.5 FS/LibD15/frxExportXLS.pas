...
type
  ...
  published
    ...
    property TruncateLongTexts: Boolean read FTruncateLongTexts write FTruncateLongTexts
      default True;
  end;
...
var
  ...
  function ConvertFormat(const fstr: string; ds: string): string;
  var
    i, err, p : integer;
    s: string;
  begin
    { todo -oDraeden: it seems that DecimalSeparator sometimes contains
      an incorrect value, maybe someone changes it at the startup ? }
          ...
          begin
            ...
            with Obj.Style.DisplayFormat do
              ...
              begin
                ...
                if (DecimalSeparator <> '') and (SysUtils.DecimalSeparator <> '') then
                  s := StringReplace(s, DecimalSeparator,
                    SysUtils.DecimalSeparator, [rfReplaceAll]);

                if FormatStr <> '' then
                begin
                  NumFmt := ConvertFormat(FormatStr, DecimalSeparator);
