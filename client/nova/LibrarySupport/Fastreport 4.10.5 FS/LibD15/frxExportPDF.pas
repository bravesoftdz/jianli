...
uses frxUtils, frxUnicodeUtils, frxFileUtils, frxRes, frxrcExports, frxPreviewPages,
  frxGraphicUtils, frxGzip, frxMD5, ActiveX, SyncObjs, Math, frxXML;
...
{$ENDIF}
  ...
  begin
    i := UpdateXRef;
    r := Format('%d 0 R ', [i]);
    s := Format('%d 0 obj'#10'<< /Type /Annot'#10'/Subtype /Link'#10 +
      '/Rect [%s %s %s %s]'#10'/Border [16 16 0]'#10'/Dest [%d /XYZ null %f null]'#10 +
...
begin
  ...
  if (Obj is TfrxCustomMemoView)
     ...
     //and TfrxCustomMemoView(Obj).WordWrap
     //and not TfrxCustomMemoView(Obj).Clipped
