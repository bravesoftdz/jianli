...
function TGmlRtf.IsSpecChar(c: AnsiChar): Boolean; cdecl;
asm
  mov   al, c
  test  al, al
  je    @@yes
...
function TGmlRtf.IsAlpha(c: AnsiChar): Boolean; cdecl;
asm
  mov   al, c
  cmp   al, 'A'
  jb    @@no
...
function TGmlRtf.IsDigit(c: AnsiChar): Boolean; cdecl;
asm
  mov   al, c
  cmp   al, '0'
  jb    @@no
...
function TGmlRtf.HexDigit(c: AnsiChar): Byte; cdecl;
asm
  mov   al, c
  cmp   al, '0'
  jb    @@no
...
function TGmlRtf.IsVisible(c: AnsiChar): Boolean; cdecl;
asm
  mov   al, c
  cmp   al, 10
  je    @@no
...
procedure TGmlRtfText.Serialize(Stream: TStream);
  ...
  function Clean(s: PAnsiChar): LongInt; cdecl;
  asm
    push   esi
    push   edi
...
procedure TGmlRtfNumber.Serialize(Stream: TStream);
  ...
  function HexDigit(x: Byte): AnsiChar; cdecl;
  asm
    mov   al, x
    cmp   al, 10
    jb    @@dec
