  ...
  begin
    ...
    repeat
      ...
      begin
        ...
        chrg.cpMin := FRichEdit.Perform(EM_FORMATRANGE, 1, Integer(@Range));
...
{$ENDIF}
            SendMessage(FRichEdit.Handle, WM_USER + 97 {EM_SETTEXTEX}, Integer(@SetText),
                Integer({$IFDEF Delphi12}PAnsiChar(AnsiStr){$ELSE}PChar(s2){$ENDIF}));
            ...
            SendMessage(FRichEdit.Handle, EM_REPLACESEL, Integer(True),
                Integer({$IFDEF Delphi12}PAnsiChar(AnsiStr){$ELSE}PChar(s2){$ENDIF}));// rich text workground
...
begin
  ...
  begin
   ...
   chrgRange.cpMax := RichEdit.GetTextLen;
    ...
    repeat
      rc := rcPage;
      chrg.cpMin := FRichEdit.Perform(EM_FORMATRANGE, 0, Integer(@Range));
...
begin
  ...
  begin
    ...
    FRichEdit.Perform(EM_SETSEL, -1, -1);// need for RE 4.1 and tables
    chrg.cpMin := 0;
    chrg.cpMax := -1;
