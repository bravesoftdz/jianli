...
// get the intercharacter spacing table and calculate the width
  ...
  try
    sz.cx := FHTMLTags.FillCharSpacingArray(FTempArray, s, FCanvas,
      FHTMLTags.Count - 1, CharSpacing, True, FUseDefaultCharset);
  finally
    FCanvas.Unlock;
  end;
...
begin
  ...
  addI := 0;
  AddNext := True;
  if ((C.Font.Charset <> DEFAULT_CHARSET) and not FUseDefaultCharset) or (Win32Platform <> VER_PLATFORM_WIN32_NT) then
  begin
  ...
  try
    ...
    repeat
      ...
      if ((C.Font.Charset = DEFAULT_CHARSET) or FUseDefaultCharset) and (Win32Platform = VER_PLATFORM_WIN32_NT) then
        if FWysiwyg then
