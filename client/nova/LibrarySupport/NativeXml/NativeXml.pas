{ unit NativeXml
  ...
  Note1: any external encoding (ANSI, UTF16, etc) is converted to an internal
  ...
  Note2: the character data is always normalized inside the document (just a $0A
  instead of $0D$0A in Windows for end-of-lines). If EolStyle = esWindows, the
  data is un-normalized before it gets consumed. If you need no un-normalisation
  (and after all it is non-optimal) you can use EolStyle = esLinux instead.

  Note: this unit is a completely redesigned implementation of legacy NativeXml.

  Author: Nils Haeck M.Sc.
  Creation Date: 10nov2010
...
// A Tag is an additional integer field of TXmlNode that can be used by the application.
{$define USETAGS}

// defne if you want to report source position by the parser. TXmlNode will have
// an additional int64 field FSourcePos
{$define SOURCEPOS}
...
const
  cQuoteCharStyleNames: array[TsdQuoteCharStyle] of AnsiChar =
    ('"', '''');
...
begin
  ...
  xeDeclaration:
    begin
      ...
      TsdContainerNode(Result).FDirectNodeCount := ReadCardinal(S);
      TsdContainerNode(Result).FValueIndex := integer(ReadCardinal(S)) - 1;
      SubCount := TsdContainerNode(Result).FDirectNodeCount;
    end;
...
begin
  ...
  // write symbol table
  ST := FDocument.SymbolTable;
  ST.SaveToStream(S, ST.PluralCount);
...
var
  ...
  begin
    if ASymbolID <= cardinal(ST.PluralCount) then
  ...
  // local
  procedure WriteElement(ANode: TXmlNode);
  begin
    // write persistent properties of containernode
...
begin
  ...
  xeCharData, xeComment, xeCData, xeWhiteSpace, xeCondSection, xeInstruction, xeStyleSheet:
    begin
      WriteCharData(ANode);
    end;
