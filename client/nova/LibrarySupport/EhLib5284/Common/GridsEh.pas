...
//      Inc(TopStart, RowHeights[IRow]);
      ...
      IsDraw := False;
      if not DoVert
        then BorderColor := OffColor
        else BorderColor := OnColor;
      IsExtent := True;
      CheckDrawCellBorder(ICol, IRow, cbtRightEh, IsDraw, BorderColor, IsExtent);
      if IsDraw then
      begin
        if IsExtent then ExtentBit := 1 else ExtentBit := 0;
        FGridLinesCache.AddLine(BorderColor,
          LeftStart + ColWidths[ICol], TopStart,
          LeftStart + ColWidths[ICol], TopStart + RowHeights[IRow] + ExtentBit
          )
      end;
      ...
      IsExtent := True;
      CheckDrawCellBorder(ICol, IRow, cbtBottomEh, IsDraw, BorderColor, IsExtent);
      if IsDraw then
      begin
        if IsExtent then ExtentBit := 1 else ExtentBit := 0;
        FGridLinesCache.AddLine(BorderColor,
          LeftStart, TopStart + RowHeights[IRow],
          LeftStart + ColWidths[ICol] + ExtentBit, TopStart + RowHeights[IRow]
          )
      end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            FactWhere := Where;
            if not CheckLine(CurCol, CurRow, True) then Inc(FactWhere.Right, DrawInfo.Horz.EffectiveLineWidth);
            if not CheckLine(CurCol, CurRow, False) then Inc(FactWhere.Bottom, DrawInfo.Vert.EffectiveLineWidth);
...
// Data cells
      DrawLinesEh(DrawInfo, goHorzLine in Options, goVertLine in Options, LeftCol, TopRow,
...
begin
  ...
  if OutBoundaryData.GetOutBoundaryRect(DrawRect, cbtRightEh) then
    DrawTopOutBoundaryData(DrawRect);
{  if OutBoundaryData.TopIndent > 0 then
  begin
    DrawRect.Top := 0;
    DrawRect.Bottom := OutBoundaryData.TopIndent;
    if OutBoundaryData.LeftTopDrawPriority = crpHorizontalDataPriorityEh
      then DrawRect.Left := 0
      else DrawRect.Left := OutBoundaryData.LeftIndent;
    if OutBoundaryData.RightTopDrawPriority = crpHorizontalDataPriorityEh
      then DrawRect.Right := ClientWidth
      else DrawRect.Right := ClientWidth - OutBoundaryData.RightIndent;
    DrawTopOutBoundaryData(DrawRect);
  end;
  if OutBoundaryData.BottomIndent > 0 then
  begin
    DrawRect.Top := ClientHeight - OutBoundaryData.BottomIndent;
    DrawRect.Bottom := ClientHeight;
    if OutBoundaryData.LeftBottomDrawPriority = crpHorizontalDataPriorityEh
      then DrawRect.Left := 0
      else DrawRect.Left := OutBoundaryData.LeftIndent;
    if OutBoundaryData.RightBottomDrawPriority = crpHorizontalDataPriorityEh
      then DrawRect.Right := ClientWidth
      else DrawRect.Right := ClientWidth - OutBoundaryData.RightIndent;
    DrawBottomOutBoundaryData(DrawRect);
  end;
  if OutBoundaryData.LeftIndent > 0 then
  begin
    DrawRect.Left := 0;
    DrawRect.Right := OutBoundaryData.LeftIndent;
    if OutBoundaryData.LeftTopDrawPriority = crpHorizontalDataPriorityEh
      then DrawRect.Top := OutBoundaryData.TopIndent
      else DrawRect.Top := 0;
    if OutBoundaryData.LeftBottomDrawPriority = crpHorizontalDataPriorityEh
      then DrawRect.Bottom := ClientHeight - OutBoundaryData.BottomIndent
      else DrawRect.Bottom := ClientHeight;
    DrawLeftOutBoundaryData(DrawRect);
  end;
  if OutBoundaryData.RightIndent > 0 then
  begin
    DrawRect.Left := ClientWidth - OutBoundaryData.RightIndent;
...
//          LineTo(Pos, ClientHeight);
          if GetMoveLineEh.Visible then
            ...
            GetMoveLineEh.StartShow(Self,
                                    ...
                                    True,
                                    DrawInfo.Vert.FullGridBoundary - DrawInfo.Vert.NullBorderIndent);
        end;
      finally
        Canvas.Pen := OldPen;
      end;
...
begin
  if (ScrollBars = ssNone) or not HandleAllocated or not Showing or PaintLocked then Exit;
  with DrawInfo do
  begin
