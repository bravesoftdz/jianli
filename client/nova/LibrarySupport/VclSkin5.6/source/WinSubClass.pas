...
//     procedure WMPaint(var msg:Tmessage);
     ...
     procedure drawCloseBtn(rc:TRect;i:integer;n:integer;hwnd:HWND);
     function BeforeProc(var Message: TMessage):boolean;override;
//     procedure AfterProc(var Message: TMessage);override;
...
procedure TSkinTab.drawCloseBtn(rc:TRect;i:integer;n:integer;hwnd:HWND);
var x,y,bw:integer;
    Acanvas:TCanvas;
    bitmap:TBitmap;
    p:TPoint;
...
//    closerect[i] := rect(x,y,x+bw,y+bw);
    ...
    x:= rc.Right-bw-4;
    y:= rc.Top+2;
    if ((rc.Bottom-rc.Top-bw-4) div 2) >0 then
    begin
      y:=y+((rc.Bottom-rc.Top-bw-4) div 2);
    end;
    acanvas:=Drawtemp.Canvas;
    //acanvas.Pen.Color:=fsd.colors[csButtonShadow] ;
...
//    ACanvas.MoveTo(X, Y);
//    ACanvas.LineTo(X+bw, Y+bw);
//    ACanvas.MoveTo(X+bw-1, Y+bw);
//    ACanvas.LineTo(X, Y+1);
//    ACanvas.MoveTo(X+1, Y);
//    ACanvas.LineTo(X+bw, Y+bw-1);

//    ACanvas.MoveTo(X, Y+bw);
//    ACanvas.LineTo(X+bw, Y);
//    ACanvas.MoveTo(X+bw-1, Y);
//    ACanvas.LineTo(X, Y+bw-1);
//    ACanvas.MoveTo(X+1, Y+bw);
//    ACanvas.LineTo(X+bw, Y+1);

    bitmap:=TBitmap.Create;
    if n=i then
      bitmap.LoadFromFile(ExtractFilePath(Paramstr(0))+'\images\activetab.bmp')
    else
      bitmap.LoadFromFile(ExtractFilePath(Paramstr(0))+'\images\notactivetab.bmp');
    GetCursorPos(p);
    ScreenToClient(hwnd,p);
    if PtInrect(rect(x,y-bw+5,x+bw,y+bw),p) then begin
      bitmap.LoadFromFile(ExtractFilePath(Paramstr(0))+'\images\clicktab.bmp');
    end;
    ACanvas.Draw(x,y-bw+5,bitmap);
    bitmap.Free;
    closerect[i] := rect(x,y-bw+5,x+bw,y+bw);
