...
uses
  ...
  ToolWin, ExtCtrls, VirtualTrees, NovaHComboBox, Buttons, NovaEdit;
type
...
type
  TFrmPermissions = class(TSimpleCRUDForm)
    lblname: TLabel;
    lblrole: TLabel;
    Label3: TLabel;
    nvedtname: TNovaEdit;
    ...
    procedure vstreePermissionChecked(Sender: TBaseVirtualTree;
      Node: PVirtualNode);
    procedure vstreePermissionFreeNode(Sender: TBaseVirtualTree;
      Node: PVirtualNode);
    procedure vstreePermissionGetNodeDataSize(Sender: TBaseVirtualTree;
      var NodeDataSize: Integer);
    procedure vstreePermissionGetText(Sender: TBaseVirtualTree;
...
function TFrmPermissions.AddTreeNode: PVirtualNode;
...
function TFrmPermissions.AddTreeNode(_permissiontype, _grade: integer; _roleid,
  _menuid, _parentid, _functionpermissionregid: int64; _orderno: single; _name,
  _parameter: String; _haspermission: boolean; _createtime,
  _createbyname: String): PVirtualNode;
var
...
function TFrmPermissions.findNodeByid(id: int64): PVirtualNode;
...
begin
  ...
  begin
    ...
    Params.ParamValues['filter_EQB_t!isactive'] :='1';
    Active := True;
...
function TFrmPermissions.getCheckStateByChilds(node: PVirtualNode): TCheckState;
var i:integer;
...
procedure TFrmPermissions.HandleAuthority(const Key: TGUID; aEnable: Boolean);
begin
  inherited;
  inherited;
...
procedure TFrmPermissions.initChildChecked(node: PVirtualNode);
...
function TFrmPermissions.locateLikeName(_name: String): PVirtualNode;
...
procedure TFrmPermissions.NovaCbbOrgChange(Sender: TObject);
begin
  inherited;
...
procedure TFrmPermissions.nvedtnameChange(Sender: TObject);
begin
  inherited;
  click :=false;
end;

procedure TFrmPermissions.nvedtnameKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TFrmPermissions.nvhelpcbbroleChange(Sender: TObject);
begin
  inherited;
...
function TFrmPermissions.recursionlocateLikeName(_name: String;
...
procedure TFrmPermissions.refreshMenupermission;
var
  Data:PPermissionData;
...
procedure TFrmPermissions.btnlocateClick(Sender: TObject);
begin
  inherited;
...
procedure TFrmPermissions.btnSaveOrgpermissionClick(Sender: TObject);
...
function TFrmPermissions.getOrgPermissionCount: Integer;
begin

end;


function TFrmPermissions.locateLikeName(_name: String; _begin: PVirtualNode): PVirtualNode;
var beginNode:PVirtualNode;
begin
  result:=nil;
  beginNode:=_begin;
  ...
  begin
    ...
    if (not assigned(result)) and assigned(beginNode.NextSibling) and (beginNode<>vstreePermission.RootNode) then
    begin
      //从父节点的下一个节点开始循环
...
procedure TFrmPermissions.refreshOrgpermission;
begin
  if nvhelpcbbrole.ItemIndex = -1 then
  begin
    SysDialogs.ShowInfo('请选择一个角色');
...
procedure TFrmPermissions.syncChildChecked(node: PVirtualNode);
var tmpchildnode:PVirtualNode;
begin
  tmpchildnode:=node.FirstChild;
  while assigned(tmpchildnode) do
...
procedure TFrmPermissions.syncParentAndChildChecked(node: PVirtualNode);
var
...
procedure TFrmPermissions.tbtnFilterClick(Sender: TObject);
begin
  inherited;
...
procedure TFrmPermissions.tbtnSaveClick(Sender: TObject);
begin
  inherited;
...
procedure TFrmPermissions.updateDSbyChecked(node: PVirtualNode);
...
procedure TFrmPermissions.vstreePermissionChecked(Sender: TBaseVirtualTree;
  Node: PVirtualNode);
begin
  inherited;
...
procedure TFrmPermissions.vstreePermissionFreeNode(Sender: TBaseVirtualTree;
  Node: PVirtualNode);
var
Data:PPermissionData;
begin
...
procedure TFrmPermissions.vstreePermissionGetNodeDataSize(
...
procedure TFrmPermissions.vstreePermissionGetText(Sender: TBaseVirtualTree;
  Node: PVirtualNode; Column: TColumnIndex; TextType: TVSTTextType;
  var CellText: string);
var
  Data: PPermissionData;
