...
//    cdsPrintTemplate: TClientDataSet;  //模板信息
//    cdsPrintTemplateitem: TClientDataSet;  //模板打印项目
...
//    begin
//      application.MessageBox('找不到打印配置数据集','错误',MB_ICONERROR);
//      Application.Terminate;
//    end;
//    LoadFromFile(localfilename);
//    if RecordCount<=0 then
//    begin
//      application.MessageBox('离线打印配置数据集为空','错误',MB_ICONERROR);
//      Application.Terminate;
//    end;
...
//    if RecordCount<=0 then
//      raise Exception.Create('离线打印模板：'+FPrintTemplateName+'的明细为空');
//  end;
//end;

//constructor TNovaPrint.create(AOwner: TComponent);  //初始化 模板信息、打印项目
//begin
...
begin
  ...
  begin
    raise Exception.Create('超出最大高度，最大高度为8.9cm');
    exit;
  end;
  ...
  begin
    ...
    begin
      raise Exception.Create('请检查打印机是否存在');
      exit;
    end;
    write(f,chr(27)+chr(64));//初始化打印机
  //  Write(f,chr(27)+chr(74)+chr(255));
    write(f,chr(27)+chr(102));//设置顺向走纸
    write(f,chr(27)+chr(51)+chr(5));//设置行高为5/180英寸
    linesize := Round(pageheight/254/5*180);
    write(f,Ansichar(27)+Ansichar(67)+Ansichar(linesize)); //页长x行
    ...
    begin
      ...
      FieldDefs.Add('bottop',ftInteger);//元素底部距顶高度
  ...
  //      printer1.FieldValues['top'] := FieldByName('top').AsInteger+33.6;
    ...
    printer1.AddIndex('Index_1', 'bottop; top', []);             { 正序 }
    printer1.AddIndex('Index_2', 'bottop; left', [ixDescending]); { 倒序 }
    //循环分行
    with printer1 do
    begin
      IndexName :='Index_2';
      first;
      maxbottop := FieldByName('bottop').AsInteger;      //第一个元素的底部高度为最大底部
      minheadtop := FieldByName('top').AsInteger;        //第一个元素的头部高度为距顶最近顶部
      Next;
      while not eof do
      begin
        if minheadtop < FieldByName('bottop').AsInteger then  //如果当前元素的底部在maxbottop和minheadtop之间（有交叉）
        begin
          edit;                                    //，则将它与前一个元素底部对齐
          FieldValues['bottop'] := maxbottop;
          if FieldByName('top').AsInteger < minheadtop then  //如果有交叉且该元素的top比minheadtop小
          begin                                              //则取该值为minheadtop
            minheadtop := FieldByName('top').AsInteger;
          end;
        end
        else if minheadtop > FieldByName('bottop').AsInteger then  //如果该元素底部比minheadtop小，则无交叉
        begin                                                      //对minheadtop与maxbottop重新赋值
          minheadtop := FieldByName('top').AsInteger;
    ...
    begin
      ...
      begin
        ...
        if(top <> pretop)then
        begin
          //高度差-字体大小产生的误差（即字体高度-24*24标准字体高度(24*0.14)）
          ...
          begin
            ...
            begin
              Write(f,AnsiChar(27)+AnsiChar(74)+AnsiChar(255));//顺向跳行
            end;
        ...
        write(f,AnsiChar(27)+AnsiChar(36)+AnsiChar(left1)+AnsiChar(left2));//绝对横向右移
        {横向位移结束}
        ...
        case FieldByName('valuetype').AsInteger of      //判断输出变量类型
          ...
          begin
            if (SingleValue<>nil) and (FieldByName('islist').AsBoolean = false) and (getSingleValue(FieldByName('itemcode').AsString,curPageNo)<>'') then //单个变量  //and (SingleValue.ContainsKey(FieldValues['itemcode']))
            begin
...
//                  SysDialogs.showMessage(printtext);
    ...
    if portname<>'' then
    begin
      write(f,chr(27)+chr(102)+chr(12));//顺向走纸一页切纸
      CloseFile(f);
    end;
  end;
...
begin
  //result:='';
//  fontpoint := round(fontheight/10/0.14);//一个点数是0.14mm
//  Write(f,AnsiChar(27)+AnsiChar(101)+AnsiChar(48)+AnsiChar(72));//设定字体大小
  if fontheight>45 then  //大于13号字体
  begin
      Write(f,AnsiChar(28)+AnsiChar(87)+AnsiChar(1));  //设定字体大小－四倍角汉字打印
  end;
  Write(f,strdata);
  Write(f,AnsiChar(28)+AnsiChar(87)+AnsiChar(0)); //恢复默认字体大小-撤消四倍角汉字打印
//  Write(f,AnsiChar(27)+AnsiChar(101)+AnsiChar(0)+AnsiChar(0));//恢复默认字体大小
end;
