unit UFrmQuerySellerTicketSummary;
...
type
  ...
  TfrmQuerySellerTicketSummary = class(TSimpleCRUDForm)
    lbl1: TLabel;
...
var
  frmQuerySellerTicketSummary: TfrmQuerySellerTicketSummary;
...
procedure TfrmQuerySellerTicketSummary.dtpComEndChange(Sender: TObject);
begin
...
procedure TfrmQuerySellerTicketSummary.dtpComStarChange(Sender: TObject);
begin
  inherited;
...
procedure TfrmQuerySellerTicketSummary.dtpenddateChange(Sender: TObject);
begin
...
//procedure TfrmQuerySellerTicketSummary.dtpstartdateChange(Sender: TObject);
//begin
//  inherited;
...
procedure TfrmQuerySellerTicketSummary.FormCreate(Sender: TObject);
...
procedure TfrmQuerySellerTicketSummary.FormShow(Sender: TObject);
begin
  inherited;
...
procedure TfrmQuerySellerTicketSummary.tbtnFilterClick(Sender: TObject);
var
  log:string;
begin
  inherited;
  ...
  begin
    ...
    Params.ParamValues['filter_LED_c!compEdate'] := FormatDateTime('yyyy-mm-dd',
      dtpComEnd.Date+1);
    Params.ParamValues['filter_EQL_u!issellby'] :='1';
    if nvhlpseller.id <= 0 then
      Params.ParamValues['filter_EQL_u!id'] := ''
    else
      Params.ParamValues['filter_EQL_u!id'] := nvhlpseller.id;
   ...
   log := '查询条件有：[售票日期]从 '+ formatdatetime('yyyy-mm-dd',dtpstartdate.Date)
    ...
    SysLog.WriteLog('报表统计->站务分公司售票员售票情况汇总表','查询',log);
  end;
